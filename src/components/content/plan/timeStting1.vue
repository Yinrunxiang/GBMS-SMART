    ...
    <el-dialog v-if="ready" class="time-setting" ref="dialog" :visible="open" :before-close="handleClose" custom-class="w-500" title="Time Contral Center">
        <div>
            <div class="day-item" w-100p>
                <day v-for="day in week" :day="day" :open="runtime[day.toLowerCase()]" @change="dayChange" @click="dayClick(day.toLowerCase())"></day>
            </div>
            <div class="m-t-15">
                <el-time-select class="m-l-20" placeholder="Start Time" v-model="runtime.up"  :picker-options="{
                                                      start: '00:00',
                                                      step: '00:10',
                                                      end: '24:00'
                                                    }">
                </el-time-select>
                <el-time-select class="m-l-20" placeholder="End Time" v-model="runtime.down"  :picker-options="{
                                              start: '00:00',
                                              step: '00:10',
                                              end: '24:00',
                              minTime: runtime.up   
      ...
      this.apiGet("device/index.php", data).then(res => {
        if (res[0]) {
          ...
          var runtime = {
            up: data.up,
            down: data.down,
            sun: data.sun_status == 1 ? true : false,
            mon: data.mon_status == 1 ? true : false,
            tues: data.tues_status == 1 ? true : false,
            wed: data.wed_status == 1 ? true : false,
            thur: data.thur_status == 1 ? true : false,
            fri: data.fri_status == 1 ? true : false,
            sat: data.sat_status == 1 ? true : false
          };
          //   var runtime = {
          //     sun: {
