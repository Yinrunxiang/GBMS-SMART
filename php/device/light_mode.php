...
switch ($action)
{
    case "insert":
        ...
        $sql="insert into light_mode (c_mode,c_watts,c_status) values ('".$c_mode."','".$c_watts."','enabled')";
    ...
    case "delete":
        ...
        for ($i = 0; $i  < count($selections); $i++) {
            ...
            $sql = " delete from light_mode where c_mode = '".$selection->c_mode."'";
            if (!mysqli_query($con,$sql))
            {
                $re = false;
    ...
    for ($i = 0; $i  < count($selections); $i++) {
        $selection = json_decode($selections[$i]);
        $sql = " update light_mode set c_status = '".$c_status."' where c_mode = '".$selection->c_mode."'";
        if (!mysqli_query($con,$sql))
        {
            $re = false;
